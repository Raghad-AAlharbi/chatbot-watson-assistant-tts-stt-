...
def watson(data):
    authenticator = IAMAuthenticator('OciUlGyWmBkqVJkbKW79vtyEesYoQplIEhoQ7pVfFl3N')
    ...
    response = assistant.message(
        assistant_id ='3c03ffab-827d-4ebc-a075-0efe695e3098',
        workspace_id = '381a86ab-cf7a-4c26-b77d-28b9bc458435',
        input={
            'message_type': 'text',
            'text': data,
        }, 
    ...
    with open('replay.txt', 'w') as file:
        file.writelines(json.dumps(response, indent=2))
    file1 = open('replay.txt', 'r')
    for x in range(16):
      txt = str(file1.readline())
    if txt != "I didn't understand. You can try rephrasing.":
      for x in range(1):
        txt = str(file1.readline())
    txt = txt.split(":")
    file1.close()
    with open('replay.txt', 'w') as finalfile:
        finalfile.writelines(txt[1])
        txt = txt[1]
    


def TTS():
    url = 'https://api.eu-gb.text-to-speech.watson.cloud.ibm.com/instances/0278b524-7146-4e68-9a25-baf43f5bbeb1'
    apikey = 'Z3aoSlWoFzdKXtsFNA4xIy_aP9TZc7bFG8_JhUfF2L5K'
